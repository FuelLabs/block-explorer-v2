...
import {
  ...
  DataItem,
  DataTitle,
  TxType, TransactionAddress, TransactionRecipientLabel, TransactionRecipientLink, TransactionTypeColumn, TransactionHashColumn, TransactionRecipientsColumn, TransactionRecipientsWrapper, TransactionsDataBoxRow, TransactionRowColumn
} from "./components";
...
export const RecentTransactions: React.FC<Props> = ({ transactions }) => {
  const sortedTransactions = transactions.sort((t1, t2) => new Date(t1.status.time).getTime() - new Date(t2.status.time).getTime() <= 0 ? 1 : -1).slice(0, 5)
